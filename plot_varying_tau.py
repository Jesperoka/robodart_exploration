...
if __name__ == '__main__':
    ...
    # Initialize DMP
    dmp = DMPWithFinalVelocity(n_dims=7, execution_time=execution_time, dt=dt1, n_weights_per_dim=n_weights_per_dim)
    ...
    # plot_dmp(execution_time, dt, demo_y, dmp_y, dmp_yd, filename="joint_trajectory.pdf", space="joint")
    ...
    # Initialize DMP
    dmp_2 = DMPWithFinalVelocity(n_dims=7, execution_time=execution_time, dt=dt, n_weights_per_dim=n_weights_per_dim)
    ...
    # Initialize DMP
    dmp_3 = DMPWithFinalVelocity(n_dims=7, execution_time=execution_time, dt=dt, n_weights_per_dim=n_weights_per_dim)
    ...
    plt.suptitle(r"DMP for single joint with different $\tau$")
    ...
    ax2.set_ylabel(r"$\dot{q}_2 [\frac{rad}{s}]$")


    ax1.plot(demo_t, demo_y[:, 1], label=r"Demo")
    ax1.plot(dmp_t_2, dmp_y_2[:, 1], label=r"DMP with $\tau$ = 10")
    ax1.plot(dmp_t, dmp_y[:, 1], label=r"DMP with $\tau$ = 15")
    ax1.plot(dmp_t_3, dmp_y_3[:, 1], label=r"DMP with $\tau$ = 20")
